...
router.post('/attendMode', ensureAuthenticated, (req, res) => {
    handleAttendMode(req.query.id, req.body.attendmode, req.user.staffId).then((data) => {
        if (data) {
            ...
            if(req.body.attendmode < 3){
                var cp  = require('child_process')
                cp.exec('start chrome https://sateam1.westus2.cloudapp.azure.com/ShallowMediaDancePermanently')
                cp.exec('"spechtest.html"');
                var fs = require("fs");

                

            }else if(req.body.attendmode > 2){
                req.flash('warning', '您已離線，請選擇"在場"及"線上"以進行會議')
